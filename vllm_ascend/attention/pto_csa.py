...
def _describe(obj, depth: int = 0):
    ...
    if isinstance(obj, torch.Tensor):
        ...
        if obj.numel() and obj.numel() <= 16 and obj.dtype in (
    ...
    if depth < 2 and hasattr(obj, "__class__"):
        ...
        for attr in ("weight", "weight_scale", "weight_offset", "bias",
                     "input_scale", "deq_scale", "scale"):
            if hasattr(obj, attr):
...
def _emit(tag: str, layer_name: str, payload: dict) -> None:
    ...
    (d / f"{tag}__{safe}.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False, default=str)
    )
    print(f"[pto-csa-probe] {tag} {layer_name} -> {d}", flush=True)


def probe_attention(impl, layer_name, *, q, ori_kv, cmp_kv, cmp_sparse_indices,
                    ori_block_table, cmp_block_table, cu_seqlens_q, seqused_kv,
                    sinks, softmax_scale, positions=None) -> None:
    """`_forward_decode` 里 vendor CSA 调用点的入参长相。"""
    if probe_dir() is None:
        return
    _emit("attn", layer_name, {
        ...
        "q": _describe(q), "ori_kv": _describe(ori_kv), "cmp_kv": _describe(cmp_kv),
        ...
        "cu_seqlens_q": _describe(cu_seqlens_q), "seqused_kv": _describe(seqused_kv),
        "sinks": _describe(sinks), "positions": _describe(positions),
    })


def probe_oproj(impl, layer_name, *, o_proj_input, cos, sin, decode_tokens,
                actual_tokens, num_tokens) -> None:
    """`forward` 里逆 RoPE + o_proj 段的入参长相（= PTO kernel 的后半段）。"""
    if probe_dir() is None:
        return
    _emit("oproj", layer_name, {
        "layer_name": layer_name,
        "compress_ratio": getattr(impl, "compress_ratio", None),
        ...
        "head_dim": getattr(impl, "head_dim", None),
        "nope_head_dim": getattr(impl, "nope_head_dim", None),
        "decode_tokens": int(decode_tokens), "actual_tokens": int(actual_tokens),
        "forward_context_num_tokens": int(num_tokens),
        "o_proj_input": _describe(o_proj_input),
        "cos": _describe(cos), "sin": _describe(sin),
...
class PtoCsaRunner:
    ...
    def ensure_compiled(self, device_id: int):
        ...
        print(f"[pto-csa] compiled T={csa.T} B={csa.B} S={csa.S} H={csa.H} "
    ...
    def weights_for(self, impl):
        ...
        if wb.dtype == torch.int8:
            ...
            print(f"[pto-csa] wo_b 由 {wb.dtype} 量化成 INT8 per-channel "
    ...
    def run_decode(self, impl, *, cos, sin):
        ...
        if _VERIFY and "inputs" not in self.stats:
            self.stats["inputs"] = verify_inputs(impl, csa, st, dict(
                ...
                idx_t=idx_t, pos_t=pos_t, cos=cos, sin=sin, cos_t=cos_t,
                sin_t=sin_t, wo_a=wo_a, wo_b=wo_b, wo_b_scale=wo_b_scale,
                sink=sink))
        ...
        _dump_once(st, dict(
            ...
            cos_t=cos_t, sin_t=sin_t, wo_a=wo_a, wo_b=wo_b, wo_b_scale=wo_b_scale,
            attn_out_full=attn_out, pto_out=out, src=src, n=n,
        ))
        return out
...
def report(layer_name: str, vendor_out, pto_out) -> None:
    ...
    if cur["steps"] in (1, 2, 5, 10, 50, 100):
        print(f"[pto-csa] {layer_name} step={cur['steps']} "
              f"max_abs={cur['max_abs']:.6g} mean_abs={cur['mean_abs']:.6g} "
              f"(vendor |mean|={cur['vendor_abs_mean']:.6g})", flush=True)
...
def _dump_once(stash, derived) -> None:
    ...
    payload = {f"stash.{k}": (v.detach().cpu() if hasattr(v, "detach") else v)
               for k, v in stash.items()
               if v is not None and (not hasattr(v, "numel") or v.numel() <= 1 << 22)}
    payload.update({f"derived.{k}": (v.detach().cpu() if hasattr(v, "detach") else v)
                    for k, v in derived.items()})
    torch.save(payload, p / "step0.pt")
    if _PENDING_VENDOR_ATTN is not None:
        torch.save(_PENDING_VENDOR_ATTN, p / "vendor_attn0.pt")
    print(f"[pto-csa] dumped step0 -> {p/'step0.pt'} "
          f"(vendor stage-1: {_PENDING_VENDOR_ATTN is not None})", flush=True)
...
def verify_inputs(impl, csa, st, d) -> dict:
    ...
    v: dict = {}
    T, S, BS, WIN = csa.T, csa.S, csa.BLOCK_SIZE, csa.WIN
    CMP_PAGE = csa.CMP_STORAGE_BLOCK_SIZE
    n = int(d["n"])
    dev = d["q_t"].device
    ...
    pairs = {
        ...
        "SOFTMAX_SCALE / softmax_scale": (round(float(csa.SOFTMAX_SCALE), 12),
                                          round(float(impl.softmax_scale), 12)),
    ...
    ref_slots, ref_lens = swa_indices_and_lens(
        pos.reshape(n, 1).cpu(), obt, block_size=BS, window=WIN)
    ...
    v["rope"] = {
        ...
        "喂给 kernel 的 cos[:HALF] 与去重参考(BF16)的差":
            float((d["cos_t"][:, :half].to(torch.bfloat16).float() - cos_ref.float()).abs().max()),
        "喂给 kernel 的 sin[:HALF] 与去重参考(BF16)的差":
            float((d["sin_t"][:, :half].to(torch.bfloat16).float() - sin_ref.float()).abs().max()),
        "BF16 化相对 vLLM FP32 表的最大差":
            float((d["cos_t"].float()[:, :half]
                   - cflat[:, 0::2][:, :half].index_select(0, src)).abs().max()),
    }
    ...
    v["weights"] = {
        "wo_a 转置回去是否逐位相同": bool(torch.equal(
            d["wo_a"].to(dev).transpose(1, 2).contiguous(), wa_v)),
        "wo_b 反量化的相对最大误差": float((deq - wb_v).abs().max() / wb_v.abs().max()),
        "attn_sink 是否逐位相同": bool(torch.equal(
            d["sink"].to(dev).float(), impl.attn_sink.detach().float().reshape(-1))),
        "q 行是否逐位相同": bool(torch.equal(
            d["q_t"].index_select(0, take), st["q"][:n])),
    }
    ...
    v["当前 token 的 KV 已在窗口内且非零"] = {
        "零行数": int((small_flat.index_select(0, cur.clamp_min(0))
                       .float().abs().amax(dim=-1) == 0).sum()),
        "token 数": n,
    }
    ...
    # 红着的检查等于没有检查。
    v["declared_gaps"] = {
        "wo_b: checkpoint 是 BF16，kernel 签名只有 INT8+per-channel scale":
            v["weights"]["wo_b 反量化的相对最大误差"],
        "RoPE 表: vLLM 用 FP32，kernel 签名要 BF16":
            v["rope"]["BF16 化相对 vLLM FP32 表的最大差"],
    }
    v["全部通过"] = (
        not bad
...
def note_forward(impl, layer_name, *, has_decode, has_prefill, md=None) -> None:
    ...
    这一条把两者分开。
    """
    key = (f"ratio={getattr(impl, 'compress_ratio', None)} "
           f"decode={bool(has_decode)} prefill={bool(has_prefill)}")
    if md is not None:
        key += (f" num_decodes={getattr(md, 'num_decodes', '?')}"
