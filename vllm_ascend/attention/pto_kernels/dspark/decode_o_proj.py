...
# TP specialization must run before importing shape-dependent kernel helpers.
# ruff: noqa: E402
import sys
...
def o_group_a2a(
    ...
    local_groups_out, gather_tid = pl.spmd_submit(
        ...
        local_groups_out,
        pl.no_dep(exchange_window),
        group_t,
        core_num=ATTENTION_PUBLISH_WORKERS,
        deps=[wait_tid],
    )

    with pl.at(
        level=pl.Level.CORE_GROUP, name_hint="o_group_a2a_complete", deps=[gather_tid], no_dep_args=[exchange_signal]
    ):
...
def l3_o_group_a2a(
    ...
    for rank in pl.range(pld.world_size()):
        ...
        l2_o_group_a2a(
            attention_grouped[rank],
            attention_local_groups[rank],
            attention_window,
            attention_signal,
            0,
            rank,
...
def o_proj_reduce_scatter(
    ...
    group_t = TP_SIZE * local_t
    _o_a_rows = (group_t + O_A_T_TILE - 1) // O_A_T_TILE
    o_b_rows = (group_t + O_B_T_TILE - 1) // O_B_T_TILE
    _o_b_group_t = o_b_rows * O_B_T_TILE
    _owner_rows = (local_t + ACT_T_TILE - 1) // ACT_T_TILE
        ...
        for local_group in pl.parallel(LOCAL_O_GROUPS):
            ...
            with pl.spmd(own_a_rows * (O_LORA // O_A_N_TILE), name_hint="tp_o_a") as pa_tid:
                ...
                for pa_k0 in pl.pipeline(O_A_K_TILE, O_GROUP_IN, O_A_K_TILE, stage=2):
                    pa_xk = pl.slice(
                        attn_2d, [O_A_T_TILE, O_A_K_TILE], [pa_src, pa_k0], valid_shape=[pa_rows, O_A_K_TILE]
                    )
    ...
    local_out, reduce_tid = pl.spmd_submit(
        ...
        local_out,
        pl.no_dep(reduce_window),
        local_t,
        core_num=O_RS_REDUCE_WORKERS,
        deps=[wait_tid],
    )
