...
def decode_o_proj_tp1(
    ...
    with pl.manual_scope():
        ...
        with pl.spmd((t_dim + QUANT_TOKEN_TILE - 1) // QUANT_TOKEN_TILE,
            ...
            # The output projection quantises through a vectorised operator, so
            # its scale comes from the vector divide. Scalar division is only
            # right for rms_norm_dynamic_quant on the QR path -- using it here
            # pushed the o_proj comparison from 0.000000% to 0.080884%.
