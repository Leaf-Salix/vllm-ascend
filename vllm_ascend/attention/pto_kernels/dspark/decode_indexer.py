        ...
        for item in pl.range(
            ...
            if logical_begin < visible_count:
                ...
                for _aiv_coeff in pl.split_aiv(2, mode=pl.SplitMode.NONE):
                    ...
                    # GlobalTensor<half> weightGm_. Rounding the coefficient
                    # measures better than rounding the scaled score -- topk
                    # 3 differing keys against 4, elementwise 87.71% against
                    # 79.42%, heads 0.144230% against 0.173913% -- so the
                    # operator does merge the two factors before the cast.
...
def indexer_qr_hadamard_mm(
    ...
    with pl.spmd(
        ...
        for idx in pl.range(qh_quant_worker, bs_heads // QH_QUANT_TILE, QH_QUANT_WORKERS):
            ...
            # npu_dynamic_quant is a vectorised operator: its scale comes out of
            # the vector divide, not the scalar unit. Scalar division belongs to
            # rms_norm_dynamic_quant on the QR path only -- routing this one
            # through it took the indexer query INT8 from 100.0000% exact down
            # to 99.9644%. indexer_quantize_query then casts to float16.
