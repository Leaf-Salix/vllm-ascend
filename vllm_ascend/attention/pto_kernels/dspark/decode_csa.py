...
def decode_csa_attn_tp1(
    ...
    """DeepSeek-V4 CSA decode attention on a single rank, hc-stack excluded.

    The caller owns the hc gates and both normalizations: ``x_normed`` arrives
    already mixed and RMS-normalized, and ``attn_out`` carries the projected
    hidden rows back without the hc residual folded in. Six caches are written
    in place (``kv_cache``, ``cmp_kv``, ``idx_kv_cache``, ``idx_kv_scale``,
    ``compress_state``, ``inner_compress_state``), so two callers must not share
    one cache world.
    """
...
def decode_csa_tp1(
    ...
    attn_out = pl.create_tensor([t_dim, D], dtype=pl.BF16)
    attn_out = decode_csa_attn_tp1(
        x_normed_t,
        wq_a, wq_b, wq_b_scale, wkv, gamma_cq,
        gamma_ckv, freqs_cos, freqs_sin, cmp_freqs_cos, cmp_freqs_sin,
        cmp_wkv, cmp_wgate, cmp_ape, cmp_norm_w, compress_state,
        compress_state_block_table, idx_wq_b, idx_wq_b_scale, weights_proj, hadamard_idx,
        inner_wkv, inner_wgate, inner_ape, inner_norm_w, inner_compress_state,
        inner_compress_state_block_table, kv_cache, cmp_kv, cmp_block_table, idx_kv_cache,
        idx_kv_scale, idx_block_table, ori_slot_mapping, window_swa_indices, cmp_slot_mapping,
        idx_slot_mapping, state_slot_mapping, inner_state_slot_mapping, position_ids, kv_seq_lens,
        attn_sink, wo_a, wo_b, wo_b_scale,
        attn_out,
    )
    hc_post(attn_out, x_hc, post_t, comb_t, x_out)
