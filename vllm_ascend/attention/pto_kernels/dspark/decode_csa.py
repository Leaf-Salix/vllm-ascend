...
INNER_ROPE_ROWS_DYN = pl.dynamic("VLLM_INNER_ROPE_ROWS_DYN")
...
def _decode_csa_attn_tp1(
    ...
    cmp_slot_mapping: pl.Tensor[[CMP_ROPE_ROWS_DYN, 2], pl.INT32],
    idx_slot_mapping: pl.Tensor[[INNER_ROPE_ROWS_DYN, 2], pl.INT32],
    cmp_query_start_loc: pl.Tensor[[QUERY_BOUNDS_DYN], pl.INT32],
    cmp_start_pos: pl.Tensor[[B_DYN], pl.INT32],
    idx_query_start_loc: pl.Tensor[[QUERY_BOUNDS_DYN], pl.INT32],
    idx_start_pos: pl.Tensor[[B_DYN], pl.INT32],
    inner_freqs_cos: pl.Tensor[[INNER_ROPE_ROWS_DYN, ROPE_HEAD_DIM], pl.FP32],
    inner_freqs_sin: pl.Tensor[[INNER_ROPE_ROWS_DYN, ROPE_HEAD_DIM], pl.FP32],
    ...
    cmp_slot_mapping.bind_dynamic(0, CMP_ROPE_ROWS_DYN)
    idx_slot_mapping.bind_dynamic(0, INNER_ROPE_ROWS_DYN)
    cmp_query_start_loc.bind_dynamic(0, QUERY_BOUNDS_DYN)
    idx_query_start_loc.bind_dynamic(0, QUERY_BOUNDS_DYN)
    cmp_start_pos.bind_dynamic(0, B_DYN)
    idx_start_pos.bind_dynamic(0, B_DYN)
    inner_freqs_cos.bind_dynamic(0, INNER_ROPE_ROWS_DYN)
    inner_freqs_sin.bind_dynamic(0, INNER_ROPE_ROWS_DYN)
    ...
    with pl.at(
        ...
        for request in pl.range(b_dim):
            cmp_start = pl.read(cmp_start_pos, [request])
            cmp_end = cmp_start + (pl.read(cmp_query_start_loc, [request + 1])
                                  - pl.read(cmp_query_start_loc, [request]))
            idx_start = pl.read(idx_start_pos, [request])
            idx_end = idx_start + (pl.read(idx_query_start_loc, [request + 1])
                                  - pl.read(idx_query_start_loc, [request]))
            pl.write(cmp_row_offsets, [request], pl.cast(cmp_prefix - cmp_start // COMPRESS_RATIO - 1, pl.INT32))
            pl.write(idx_row_offsets, [request], pl.cast(idx_prefix - idx_start // COMPRESS_RATIO - 1, pl.INT32))
        ...
        for token in pl.range(t_dim):
            ...
            active_position = -1
            valid = 0
            # Slot sentinel is the native write contract, including padded rows.
            if pl.read(ori_slot_mapping, [token, 0]) >= 0 and pl.read(ori_slot_mapping, [token, 1]) >= 0:
                valid = 1
    ...
    with pl.spmd(
        ...
        for token in pl.range(worker, t_dim, TP1_CSA_WB_WORKERS):
            physical_page_i32 = pl.read(ori_slot_mapping, [token, 0])
            intra = pl.cast(pl.read(ori_slot_mapping, [token, 1]), pl.INDEX)
