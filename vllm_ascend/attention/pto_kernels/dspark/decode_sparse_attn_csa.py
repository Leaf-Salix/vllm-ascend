...
def sparse_attn_csa(
    ...
    ori_kv_flat = pl.reshape(
        ori_kv,
        [ori_block_num * page_rows, HEAD_DIM],
    )
    ...
    cmp_kv_flat = pl.reshape(
        cmp_kv,
        [cmp_block_num * page_rows, HEAD_DIM],
    )
...
def sparse_attn_csa_tp1_vllm(
    q: pl.Tensor[[T_DYN, H, HEAD_DIM], pl.BF16],
    kv_cache_pages: pl.Tensor[[VLLM_KV_CACHE_PAGE_NUM_DYN, VLLM_PAGE_ROWS, 1, HEAD_DIM], pl.BF16],
    ori_block_table: pl.Tensor[[B_DYN, VLLM_ORI_TABLE_WIDTH_DYN], pl.INT32],
    cmp_kv_pages: pl.Tensor[[VLLM_CMP_KV_PAGE_NUM_DYN, VLLM_PAGE_ROWS, 1, HEAD_DIM], pl.BF16],
            ...
            for column in pl.range(WIN):
                ...
                if valid > 0 and column < window_len:
                    ...
                    if page_i32 > 0:
                        ...
                        physical_row = page * VLLM_PAGE_ROWS + logical_row % VLLM_PAGE_ROWS
                pl.write(
                    window_indices,
                    [token, column],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cache-window-replacement-fixture",
        action="store_true",
        default=False,
    ...
    print(
        f"compress_ratio={COMPRESS_RATIO} -> TOPK={TOPK} SPARSE_BLOCKS={SPARSE_BLOCKS} PADDED_TOPK={PADDED_TOPK}",
        flush=True,
    )
    ...
    result = run(
        ...
        specs=build_tensor_specs(
            ...
            start_pos=start_pos,
            batch=args.batch,
            block_holes_fixture=args.block_holes_fixture,
        ),
