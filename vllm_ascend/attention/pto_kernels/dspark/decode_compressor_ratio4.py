...
VLLM_COMPRESS_STATE_PAGE_NUM_DYN = pl.dynamic("VLLM_CSA_COMPRESS_STATE_PAGES_DYN")
...
def compressor_ratio4_pool_projected_vllm(
    ...
    compress_state_block_table: pl.Tensor[[B_DYN, VLLM_STATE_TABLE_BLOCKS_DYN], pl.INT32],
    ape: pl.Tensor[[COMPRESS_RATIO, OUT_DIM], pl.FP32],
    position_ids: pl.Tensor[[T_DYN], pl.INT32],
                    ...
                    for h0 in pl.range(0, HEAD_DIM, POOL_HEAD_TILE):
                        ...
                        for state_idx in pl.range(STATE_LEN - 1):
                            ...
                            if logical_position >= 0 and logical_position < first_position:
                                logical_page = logical_position // VLLM_COMPRESS_STATE_LIVE_ROWS
                                ...
                                if physical_page_i32 > 0:
                                    ...
                                    intra = logical_position % VLLM_COMPRESS_STATE_LIVE_ROWS
                                    state_row = physical_page * VLLM_COMPRESS_STATE_PAGE_ROWS + intra
                                    value = compress_state_flat[
                                        state_row : state_row + 1,
                                        state_half + h0 : state_half + h0 + POOL_HEAD_TILE,
                                    ]
...
def compressor_ratio4_cache_write_vllm(
    ...
    cmp_kv_pages: pl.Tensor[[VLLM_CMP_KV_PAGE_NUM_DYN, VLLM_KV_PAGE_ROWS, 1, HEAD_DIM], pl.BF16],
    cmp_block_table: pl.Tensor[[B_DYN, VLLM_CMP_TABLE_BLOCKS_DYN], pl.INT32],
    compress_state_block_table: pl.Tensor[[B_DYN, VLLM_STATE_TABLE_BLOCKS_DYN], pl.INT32],
    ape: pl.Tensor[[COMPRESS_RATIO, OUT_DIM], pl.FP32],
    kv_proj_pad: pl.Tensor[[BS_PAD, OUT_DIM], pl.FP32],
    score_proj_pad: pl.Tensor[[BS_PAD, OUT_DIM], pl.FP32],
    ...
    with pl.spmd(
        ...
        partial_sq = pl.full([1, RMS_PAD_TILE], dtype=pl.FP32, value=0.0)
        for k0 in pl.range(0, HEAD_DIM, HEAD_TILE):
            values = pooled_kv[row_begin : row_begin + RMS_PAD_TILE, k0 : k0 + HEAD_TILE]
            partial_sq = pl.add(
                partial_sq,
        ...
        for k0 in pl.range(0, NOPE_HEAD_DIM, HEAD_TILE):
            values = pooled_kv[row_begin : row_begin + RMS_PAD_TILE, k0 : k0 + HEAD_TILE]
            gamma = pl.cast(
                norm_w_2d[:, k0 : k0 + HEAD_TILE],
                target_type=pl.FP32,
            )
            normed_kv[row_begin : row_begin + RMS_PAD_TILE, k0 : k0 + HEAD_TILE] = pl.col_expand_mul(
                pl.row_expand_mul(values, inv_rms), gamma
            )

        rope_values = pooled_kv[row_begin : row_begin + RMS_PAD_TILE, NOPE_HEAD_DIM:HEAD_DIM]
        rope_gamma = pl.cast(
            norm_w_2d[:, NOPE_HEAD_DIM:HEAD_DIM],
            target_type=pl.FP32,
        )
        rope_normed = pl.col_expand_mul(
            pl.row_expand_mul(rope_values, inv_rms),
            rope_gamma,
        )
        rope_ones = pl.full(
            [RMS_PAD_TILE, ROPE_HEAD_DIM],
        ...
        normed_kv[row_begin : row_begin + RMS_PAD_TILE, NOPE_HEAD_DIM:HEAD_DIM] = rotated
        ...
        for inner in pl.range(rows):
            ...
            if valid != 0 and (position + 1) % COMPRESS_RATIO == 0:
                ...
                if physical_page_i32 > 0:
                    ...
                    row = normed_kv[token : token + 1, 0:HEAD_DIM]
        ...
        for s in range(S):
            ...
            for state_idx in range(STATE_LEN):
                ...
                if first_pos <= logical_pos <= token_pos:
                    ...
                    score = (
                        score_proj[overlay, half : half + HEAD_DIM] + ape[logical_pos % ratio, half : half + HEAD_DIM]
                    )
...
def build_tensor_specs(start_pos=None, batch=B):
    ...
    return [
        ...
        TensorSpec(
            "cmp_kv_cache", [CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], torch.bfloat16, init_value=init_cmp_kv_cache
        ),
        TensorSpec("position_ids", [batch * S], torch.int32, init_value=lambda: init_position_ids().reshape(-1)),
        TensorSpec(
            "cmp_slot_mapping", [batch * S], torch.int64, init_value=lambda: init_cmp_slot_mapping().reshape(-1)
        ),
        TensorSpec(
            "state_slot_mapping", [batch * S], torch.int64, init_value=lambda: init_state_slot_mapping().reshape(-1)
        ),
    ]
