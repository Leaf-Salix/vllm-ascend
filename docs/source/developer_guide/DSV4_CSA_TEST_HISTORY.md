...
## G9：Indexer对齐与剩余误差（2026-09-24）

八组模块诊断和两种长度完整单层Graph完成。完整CSA输出误差降至8K 0.7363%、128K 0.7750%。
仍有TopK和同输入attention残差，继续按原生内部舍入边界验证。
详细口径、对照表及证据见[Indexer报告](DSV4_CSA_INDEXER_20260924.md)。

## G10：打分与 inverse RoPE 边界（2026-09-24）

完整单层 Graph 输出误差降至 8K 0.4447%、128K 0.3780%；尚未达到少数 ULP 的目标。
同轮候选延迟约增加 0.34%/4.19%。模块混合诊断与完整结果分开记录。
详见[边界对齐报告](DSV4_CSA_SCORE_BOUNDARIES_20260924.md)。

## G11：Softmax 累计最大值（2026-09-24）

无注入完整单层输出误差降至 8K 0.3341%、128K 0.2242%；延迟基本持平，仍需继续对齐。
单因素诊断、失败修正与完整结果见[Softmax 报告](DSV4_CSA_SOFTMAX_20260924.md)。
