...
搬运那 20.8 ms 可以在转换层内修掉(改成不触发去 stride),修完约 7.2 ms/步。
但索引换算里约 1.0~1.5 ms/步**消不掉**,因为两套寻址约定之间必须有人翻译,
且必须做在图内 —— 图重放时 Python 不执行,翻译结果没法预先算好塞进去。
这是请 lib 侧提供入参对齐版本的动因。

## 8. 仍待补的信息

以下几项**只能从活跃张量上读**,现有转储(记了 shape/dtype/contig/stride)不够,
需要再跑一轮带数值转储的:

- 六份 cache 的 `storage_offset` 与 `data_ptr` 关系,特别是 indexer key 与 scale
  是否始终共享同一个 allocation 与物理页编号
- 主压缩器 state 每页 16 行中后 8 行的归属(padding、保留,还是另有用途)
- **state block table 记录的是绝对历史页还是固定环形槽** —— 这条最关键,
  答错不会报错,会在多步 decode 中静默写错状态
- 各块表的实际示例值,以及跨 8 行 / 16 行 / 128 行边界时的更新行为
- 五类 slot mapping 的跨页边界实例

另需一份覆盖连续两个 decode 步的 fixture,含各类边界(8 行逻辑页边界、16 行环回绕、
128 行 KV 页边界、indexer 页切换、inactive token、多请求不同物理页)。
