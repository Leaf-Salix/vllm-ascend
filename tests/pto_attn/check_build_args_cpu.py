...
class Lin:
    def __init__(self, *shape, dtype=torch.bfloat16, scale=None):
        self.weight = (
            torch.randn(*shape).to(dtype) if dtype != torch.int8 else torch.randint(-127, 127, shape, dtype=torch.int8)
        )
...
class Indexer:
    def __init__(self, quantized=True):
        ...
        self.wq_b = (
            Lin(M.q_lora_rank, K.IDX_N_HEADS * K.IDX_HEAD_DIM, dtype=torch.int8, scale=K.IDX_N_HEADS * K.IDX_HEAD_DIM)
            if quantized
            else Lin(K.IDX_N_HEADS * K.IDX_HEAD_DIM, M.q_lora_rank)
        )
...
class Impl:
    def __init__(self, quantized: bool):
        ...
        if quantized:
            ...
            self.wq_b = Lin(M.q_lora_rank, K.H * K.HEAD_DIM, dtype=torch.int8, scale=K.H * K.HEAD_DIM)
...
index_key = torch.as_strided(
    index_parent,
    (64, 128, 1, K.IDX_HEAD_DIM),
    (16640, K.IDX_HEAD_DIM, K.IDX_HEAD_DIM, 1),
    0,
)
index_scale = torch.as_strided(
    index_parent.view(torch.float16),
    (64, 128, 1, 1),
    (8320, 1, 1, 1),
    8192,
)
...
check(
    ...
    all(batch <= K.B and batch * K.S <= K.T and (batch * K.S) % 4 == 0 for batch in TARGET_BATCHES),
...
want = {
    "x_normed": [RT, D],
    "attn_out": [RT, D],
    "freqs_cos": [ROPE_ROWS, 64],
    "cmp_freqs_cos": [ROPE_ROWS, 64],
    "position_ids": [RT],
    "token_valid": [RT],
    ...
    "wq_b": [M.q_lora_rank, K.H * K.HEAD_DIM],
    "wq_b_scale": [K.H * K.HEAD_DIM],
    "idx_wq_b": [M.q_lora_rank, K.IDX_N_HEADS * K.IDX_HEAD_DIM],
    "idx_wq_b_scale": [K.IDX_N_HEADS * K.IDX_HEAD_DIM],
    "wo_b": [D, K.O_GROUPS * K.O_LORA],
    "wo_b_scale": [D],
    "wq_a": [D, M.q_lora_rank],
    "wkv": [D, K.HEAD_DIM],
    "cmp_ape": [4, K.MAIN_OUT_DIM],
    "inner_ape": [4, K.INNER_OUT_DIM],
    "attn_sink": [K.H],
}
...
check("main state aliases its parent", by["compress_state_pages"].data_ptr() == main_state.data_ptr())
check("raw KV aliases its parent", by["kv_cache_pages"].data_ptr() == raw_parent.data_ptr())
check("compressed KV aliases its parent", by["cmp_kv_pages"].data_ptr() == cmp_parent.data_ptr())
check(
    "main state and compressed KV share pages", by["compress_state_pages"].data_ptr() == by["cmp_kv_pages"].data_ptr()
)
...
check(
    "positions alias vLLM INT64",
    by["position_ids"].data_ptr() == metas[0].decode.input_positions.data_ptr()
    and by["position_ids"].dtype == torch.int64,
)
check(
    "RoPE aliases persistent FP32 table",
    by["freqs_cos"].data_ptr() == rope_cos.data_ptr() and by["freqs_cos"].dtype == torch.float32,
)
