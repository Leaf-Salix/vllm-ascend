...
### 2026-09-24 FP16 打分假设：**未被证实，已回退**

任务 `task_20260924_232201_252462123595`（AB）/ `task_20260924_233515_280552912429`（探针）。

| 指标 | 批次 19 | FP16 打分 |
|------|---------|----------|
| topk 分歧 | 4 个 key，token **[0, 7, 9, 11]** | 4 个 key，token **[0, 3, 8, 11]** |
| `heads` | 0.172731% | 0.173931% |
| `output` | 0.528919% | 0.511334% |
| indexer q INT8 | 100% 逐位 | 100% 逐位 |

**分歧数量完全没变，只是换了一批 token。** 这是"抛硬币"的特征——扰动量级正好能翻动
边界决策，但没有向原生收敛。若假设正确，应看到分歧数下降。

`output` 略好而 `heads` 略差，两者方向相反，说明差异来自"哪几个近似打平恰好翻到哪边"，
不是系统性改善。**已回退**，并把这个否定性结论写进代码注释，防止重复尝试。

### topk 收口结论

探针已证明 `npu_lightning_indexer_quant` 的**每一个输入**都与原生逐位相同
（196608 个 INT8 码字 + 1536 个 FP16 dequant scale + key cache + weights）。
因此剩余的 4 个 key 分歧**只存在于该融合算子内部**。

我们的打分实现与它的差异在 **1e-4 相对量级**（足以翻动 1e-3 的边界间隙），
且本质随机。候选排列（累加精度、`k_scale` 施加位置、`q_scale`×`weights` 是否预乘、
ReLU 位置）构成一个较大的搜索空间，**每次尝试都是一轮卡时的抽奖，没有验证手段**。
除非拿到该算子的内部实现，否则这 4 个 key（12288 次选择中的 0.03%）是硬底噪。

### heads 的最终归因

| 成分 | 值 | 性质 |
|------|-----|------|
| topk 一致的 20 个 token | 0.0299% | attention 核心自身的 FP32 累加顺序，融合算子 vs 分解实现 |
| topk 分歧的 4 个 token | 0.4034% | 打分算子内部实现差异 |
| 合成 | **0.1727%** | — |

两者都已定位到融合算子内部，**dtype 边界对齐这条路已经走到头**。

### 2026-09-24 最终状态（回退确认，task_20260924_233837_287313619080，exit=0）

`output` 精确回到 **0.528919%**，与批次 19 逐位一致，确认 FP16 实验已完全回退。

| 中间量 | 值 | 状态 |
|--------|-----|------|
| `index_key` | **0.000000%** | 逐位一致 |
| `index_scale` | **0.000000%** | 逐位一致 |
| `qr_int8` | **0.000000%** | 逐位一致（24576 码字）|
| indexer q INT8 | **0.000000%** | 逐位一致（196608 码字）|
| indexer q scale | **0.000000%** | 逐位一致（1536 个 FP16 值）|
| `main_state` / `inner_state` | 0.00003% | 底噪 |
| `compressed` | 0.000113% | 底噪 |
| `q` | 0.002194% | 99.998% 逐位 |
| `raw` / `kv_after_rope` | 0.007072% | `wkv` 矩阵乘累加顺序 |
| `heads` | 0.172731% | 50.89% 逐位 |
| `output` | **0.528919%** | — |

### inverse RoPE 不是 attention 核心误差的来源（离线确认）

仅取 topk 一致的 20 个 token，按列拆分：

| 列范围 | rel_l2 | 逐位一致 |
|--------|--------|---------|
| 全部 512 列 | 0.0302% | 59.7% |
| RoPE **未触及**的 448 列 | 0.0292% | 59.66% |
| RoPE 触及的 64 列 | 0.0398% | 60.05% |

两者的逐位一致率几乎相同，误差在所有列上均匀存在。**inverse RoPE 基本不贡献**，
误差产生于 attention 核心内部的 PV 归约与 softmax。

### 收口：dtype 边界对齐已走到尽头

会话内 `output` 从 1.4353% 降到 **0.5289%**，五个量达成逐位一致。剩余两块都已
定位到融合算子内部：

| 成分 | 值 | 需要什么才能继续 |
|------|-----|-----------------|
| topk 分歧（4 key / 12288） | heads 的 0.4034%×4 token | `npu_lightning_indexer_quant` 的内部累加顺序与精度 |
| attention 核心 | heads 的 0.0299%×20 token | `npu_sparse_attn_sharedkv` 的内部分块与累加顺序 |
| `raw` / `q` / `compressed` 的底噪 | 1e-4 ~ 1e-2 % | 我方矩阵乘 tiling 与原生融合算子逐位一致 |
